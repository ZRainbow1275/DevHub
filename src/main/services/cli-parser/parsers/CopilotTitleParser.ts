...
const DEFAULT_COPILOT_RULES: readonly TitlePatternRule[] = titleRulesConfig.copilot.map(rule => titlePatternRuleSchema.parse(rule))

function compileTitleRules(rules: readonly unknown[]): CompiledTitleRule[] {
  return rules.map((input, index) => {
    const rule = titlePatternRuleSchema.parse(input)
    if (rule.tool !== 'copilot') throw new Error(`E_VALIDATION:title rule tool mismatch at index ${index}`)
    try {
...
export class CopilotTitleParser implements IParser {
  private rules = compileTitleRules(DEFAULT_COPILOT_RULES)
  readonly descriptor: ParserDescriptor = { tool: 'copilot', strategy: 'line', priority: 94, enabled: true }
  ...
  reloadRules(rules: readonly unknown[]) {
    this.rules = compileTitleRules(rules)
    return { success: true, tool: 'copilot' as const, applied: this.rules.length }
  }
