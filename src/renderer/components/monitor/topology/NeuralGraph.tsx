...
export const NeuralGraphWithControls = memo(function NeuralGraphWithControls({
  ...
  useEffect(() => {
    ...
    const engine = new NeuralGraphEngine(engineContainerRef.current, config)
    engineRef.current = engine
    return () => {
      engine.destroy()
      engineRef.current = null
  ...
  useEffect(() => {
    if (!engineRef.current) return
    engineRef.current.onNodeClick((node) => {
    ...
    <div className={`h-full flex flex-col bg-surface-950 ${className}`}>
        ...
        <div className="flex-shrink-0 flex items-center gap-4 px-4 py-2 bg-surface-900/50 border-b border-surface-700/30">
          {stats.map((s) => (
            <div key={s.label} className="flex items-center gap-2 text-xs text-text-muted">
              <span className="w-2 h-2" style={{ backgroundColor: s.color, borderRadius: '1px' }} />
              <span>
      ...
      <div className="flex-1 relative overflow-hidden">
        {isEmpty ? (
          ...
          <>
            <div ref={engineContainerRef} className="w-full h-full" />

            {/* Mini detail overlay */}
            {selectedNode && (
              <div
                className="absolute right-2 top-2 w-56 bg-surface-900 border-l-3 border-accent p-3 z-20 text-xs"
                style={{ borderRadius: '2px' }}
              >
